...
namespace Backend.GameSystems.DynamicEvent
{
    ...
    /// </summary>
    public sealed class DynamicEventSystem : SingletonGameObject<DynamicEventSystem>
    {
            ...
            bool useProbabilityRoll)
        {
            ...
            Debug.Log(
                $"[DynamicEventSystem] Started {template.EventId} at floor {floor} (guaranteed={!useProbabilityRoll})");
        ...
        private async UniTaskVoid RunEventFlowAsync()
        {
            ...
            try
            {
                ...
                Debug.Log(
                    $"[DynamicEventSystem] Resolved {_pendingTemplate.EventId}: choice={choiceId}, outcome={outcome}");
            }
            finally
            {
                _manualChoiceTcs = null;
