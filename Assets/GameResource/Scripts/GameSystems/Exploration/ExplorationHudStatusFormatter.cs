    ...
    public static class ExplorationHudStatusFormatter
    {
        public static string Build(ExplorationState state)
        {
            ...
            return
                ...
                $"골드 {state.Gold} · 마나 {state.ManaShards + (meta?.ManaShards ?? 0)} · 유산 {meta?.LegacyPoints ?? 0} · {ScriptoriumManager.GetDisplayLabel()} · {LlmQualitySettings.GetDisplayLabel()} · {LogFrequencySettings.GetDisplayLabel()} · {OfflineSummaryDetailSettings.GetDisplayLabel()} · {DynamicEventAutoPolicySettings.GetDisplayLabel()} · {GoldenEventSettings.GetDisplayLabel()}\n" +
