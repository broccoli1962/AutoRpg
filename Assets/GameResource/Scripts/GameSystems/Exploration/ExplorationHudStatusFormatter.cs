    ...
    public static class ExplorationHudStatusFormatter
    {
        public static string Build(ExplorationState state)
        {
            ...
            return
                ...
                $"골드 {state.Gold} · 마나 {state.ManaShards + (meta?.ManaShards ?? 0)} · 명성 {state.Reputation + (meta?.Reputation ?? 0)} · 유물 {state.RelicFragments + (meta?.RelicFragments ?? 0)} · 유산 {meta?.LegacyPoints ?? 0} · {InnManager.GetDisplayLabel()} · {BlacksmithManager.GetDisplayLabel()} · {TrainingGroundManager.GetDisplayLabel()} · {ScriptoriumManager.GetDisplayLabel()} · {LlmQualitySettings.GetDisplayLabel()} · {LogFrequencySettings.GetDisplayLabel()} · {OfflineSummaryDetailSettings.GetDisplayLabel()} · {DynamicEventAutoPolicySettings.GetDisplayLabel()} · {GoldenEventSettings.GetDisplayLabel()}\n" +
